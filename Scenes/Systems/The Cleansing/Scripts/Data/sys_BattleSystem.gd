...
var playerHasSent:bool
var enemyHasSent:bool
...
func Selection(playerSent:bool, action:MenuActions, option:int=-1, detail:int=-1):
...
func RoundStep():
	...
	match currentStep:
		...
		BattleSteps.ActionSelect:
			if playerHasSent and enemyHasSent:
		...
		BattleSteps.RoundStart:
			...
			if playerBattler.pGuard > 0:
				EventBus.emit_signal("GuardChanged", playerBattler, false, true)
			playerBattler.mGuard += playerGuards[1]
			if playerBattler.mGuard > 0:
				EventBus.emit_signal("GuardChanged", playerBattler, true, true)
			var enemyGuards:Array[int] = BattleCalcs.ArmorCalc(enemyBattler)
			enemyBattler.pGuard += enemyGuards[0]
			if enemyBattler.pGuard > 0:
				EventBus.emit_signal("GuardChanged", enemyBattler, false, true)
			enemyBattler.mGuard += enemyGuards[1]
			if enemyBattler.mGuard > 0:
				EventBus.emit_signal("GuardChanged", enemyBattler, true, true)
		...
		BattleSteps.BeforeAction:
			...
			if user.currentTactic != Enums.Tactics.None and user.health > 0:
				...
				if user.pGuard > 0:
					EventBus.emit_signal("GuardChanged", user, false, false)
				user.pGuard = 0
				if user.mGuard > 0:
					EventBus.emit_signal("GuardChanged", user, true, false)
				user.mGuard = 0
		...
		BattleSteps.DuringAction:
			...
			if playerBattler.health <= 0 and !playerBattler.fainted:
				playerBattler.fainted = true
				EventBus.emit_signal("FaebleFainted",playerBattler)
				await get_tree().create_timer(0.5).timeout
			if enemyBattler.health <= 0 and !enemyBattler.fainted:
		...
		BattleSteps.AfterAction:
			...
			if user.switching:
				...
				EventBus.emit_signal("FaebleSwitched",user.currentFaeble)
		...
		BattleSteps.AfterAll:
			...
			if playerBattler.fainted == true:
				currentStep = BattleSteps.Switch
			if enemyBattler.fainted == true:
				currentStep = BattleSteps.Switch
			EventBus.emit_signal("BattleStateChanged", currentStep)
			return
			#RoundStep()
		...
		BattleSteps.Switch:
			...
			if playerBattler.switching or enemyBattler.switching:
				if playerBattler.switching:
					...
					EventBus.emit_signal("FaebleSwitched",playerBattler.currentFaeble)
				...
				if enemyBattler.switching:
					...
					EventBus.emit_signal("FaebleSwitched",enemyBattler.currentFaeble)
...
func FaintSwitch(battler:BattlerData,detail:int,wait:bool=false):
