...
@export var hpDisplay:HealthDisplay
@export var buildupDisplay:StatusDisplay
@export var statusLabel:Label3D
@export var nameLabel:Label3D
@export var levelLabel:Label3D

#func _ready(): #Move these signals up to UI Subsequencer
...
func SetupBookmark(target:BattlerData):
	hpDisplay.MaxHealthReset(target.instance.maxHP)
	hpDisplay.SetHealthDisplay(target.instance.maxHP, target.instance.currentHP)
	buildupDisplay.MaxBuildupReset(target.instance.maxBuildup)
	buildupDisplay.SetStatusDisplay(
		target.instance.maxBuildup,
		target.instance.currentBuildup,
		target.instance.currentBuildupTarget)
	#BuildupChanged(target)
	nameLabel.text = target.instance.name
	statusLabel.text = ""
	levelLabel.text = "Ch. " + str(target.instance.chapter)
