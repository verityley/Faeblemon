...
@export var choiceManager:Node3D
...
func ProgressConversation():
	...
	if currentMessage.hasChoices == true:
		if choiceManager.choicesShown == true:
			print("Choices already offered.")
			return
		choiceManager.PresentChoices(currentMessage.choiceText.size(), currentMessage)
		choosing = true
...
func ProgressChoices(choice:int):
	while logIndex < logSize:
		await ScrollUp()
	var currentMessage:Message = messageLog.back()
	var nextMessage:Message
	nextMessage = currentMessage.choiceTargets[choice]
	NewMessage(nextMessage)
	choosing = false
...
func NewMessage(newMessage:Message):
	...
	logSize += 1
	logIndex += 1
	if newMessage.hasChoices == true:
		if choiceManager.choicesShown == true:
			print("Choices already offered.")
			return
		choiceManager.PresentChoices(newMessage.choiceText.size(), newMessage)
		choosing = true
		pass #if choices, types out new message then presents choices, sends to separate wait-state
	scrolling = false
