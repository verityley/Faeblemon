...
func BattlerSetup(stageSystem:StageSystem):
	...
	healthDisplay.position = faebleObject.position
	...
	if !player:
		...
		manaDisplay.rotation.y = deg_to_rad(180)
...
func ChangeHealth(amount:int, heal:bool=false):
	if heal:
		if health + amount <= faebleInstance.maxHP:
			healthDisplay.SetHealthDisplay(faebleInstance.maxHP, health+amount)
			health += amount
		elif health + amount > faebleInstance.maxHP:
			healthDisplay.SetHealthDisplay(faebleInstance.maxHP, faebleInstance.maxHP)
			health = faebleInstance.maxHP
		return
	if health - amount > 0:
		healthDisplay.SetHealthDisplay(faebleInstance.maxHP, health-amount)
		health -= amount
	elif health - amount <= 0:
		healthDisplay.SetHealthDisplay(faebleInstance.maxHP, 0)
		health = 0
		#Defeat Battler Function
