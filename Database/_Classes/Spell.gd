...
@export var empowerment:Spell #The alternate move to use if this move is empowered
...
func DefaultDamage(user:BattlerData, target:BattlerData, range:int, aura:Aura = null):
	...
	EventBus.emit_signal("AttackResults", target, results)
...
func DefaultStatus(user:BattlerData, target:BattlerData, range:int, aura:Aura = null):
	...
	EventBus.emit_signal("StatusResults", target, results, buildup, buildupType)
...
func DefaultBuffs(target:BattlerData):
	if target.buffStages[buffStat]+buffAmount > BattleCalcs.maxStages:
		print("The target stat cannot be raised any higher!")
		return #Not yet implemented, signal for max stages
	if target.buffStages[buffStat]-buffAmount < -BattleCalcs.maxStages:
		print("The target stat cannot be reduced any lower!")
		return #Not yet implemented, signal for max stages
	...
	if buffAmount > 0:
		prints(target.instance.name,"'s",Enums.BuffableAttrs.keys()[buffStat+1],"rose by",buffAmount,"stages!")
	elif buffAmount < 0:
		prints(target.instance.name,"'s",Enums.BuffableAttrs.keys()[buffStat+1],"lowered by",buffAmount,"stages!")
